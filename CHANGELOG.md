...
- **Pool introspection and recovery.** `pool-status` returns a credential-free
  snapshot of the active pool registry (`:refs`, `:open?`, `:db-spec`).
  `remove-from-pool` retires a generation, so the next `connect-store` builds a
  fresh one while live stores keep working; the retired datasource closes after
  its final original holder releases it and is visible through
  `retired-pool-status`. `:validate-pool? true` probes an existing pool and
  rebuilds it if something closed it out of band. `release-pool!` is the
  spec-level counterpart of `release`.
...
- **Failed pool construction is cleaned up.** A datasource whose initial JDBC
  checkout fails is closed before the failure escapes, so retry loops do not
  accumulate c3p0 registry entries and helper threads.
- **Pool diagnostics redact credentials.** Usernames, passwords, access tokens,
  and URI user-info are omitted from registry snapshots, including credentials
  embedded inside `:jdbcUrl`.
